...
Applications can implement agent functionalities notably using the [`@tairu/react` package](./api/react/index.md) as a generic React agent, or the [`@tairu/react-dom` package](./api/react-dom/index.md) for Web applications.
